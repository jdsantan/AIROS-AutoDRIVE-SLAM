...
Para que tu repositorio destaque, te recomiendo subir las siguientes imágenes a una carpeta llamada media y enlazarlas:

mapa_final.png: Captura del mapa limpio del circuito óvalo.

error_ghosting.png: Captura del mapa desdoblado (para mostrar el problema inicial sin odometría).

tf_tree.png: Diagrama del árbol de transformadas.

demo_mapeo.gif: Un video corto mostrando RViz2 construyendo el mapa mientras conduces.
...
Solución: Modificación del bridge de entrada para reemplazar comas por puntos dinámicamente.

3. Odometría Dinámica y Árbol TF (odom_publisher.py)
Problema: El mapa se duplicaba en las curvas (efecto "fantasma") porque el sistema no sabía cuánto giraba el carro.

Solución: Se creó un nodo que fusiona la posición del IPS con la orientación real del IMU (Cuaterniones). Esto genera un árbol TF dinámico: map -> odom -> f1tenth_1.

4. Sincronización Temporal (lidar_republisher.py)
Problema: slam_toolbox descartaba los datos del láser porque los relojes de Windows y Ubuntu no estaban perfectamente sincronizados.

Solución: Un nodo "interceptor" que recibe el LiDAR, borra el tiempo de Windows y le estampa la hora exacta de Linux antes de republicarlo en /lidar_fixed.

🛠️ Instalación y Dependencias
Es obligatorio aislar las librerías para evitar conflictos con el sistema base:
...
Ejecuta cada comando en una terminal diferente. Importante: Siempre activa el entorno virtual y exporta el PYTHONPATH.
...
(Conecta el simulador en Windows ahora).
...
ros2 run slam_toolbox async_slam_toolbox_node --ros-args \
  ...
  -r /scan:=/lidar_fixed
💾 Guardar el Mapa
Cuando termines de recorrer el circuito con W, A, S, D en Windows, guarda el resultado:
